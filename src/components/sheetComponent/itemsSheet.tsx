...
export default function ItemsSheet(props: ItemsSheetProps) {
    ...
    if (!items || items.length === 0) return <p>Aucun objet dans l'inventaire.</p>;
    ...
    return (
        <>
            ...
            <div className="w-full rounded-[3px] bg-primary p-[16px]">
                <div className="w-full mb-[32px] flex">
                    {items.map((item, index) => {
                        ...
                        return (
                            <div
                                key={item.id}
                                className={`relative item-wrapper ${paddingClass} cursor-pointer 
                                    ...
                                    <div className="absolute popover bottom-full text-text left-1/2 -translate-x-1/2 mb-2 z-10 flex flex-col items-center">
                                        <div className="w-[250px] flex flex-row gap-y-[40px] p-[32px] bg-text shadow-lg rounded-[3px]">
                                            <span className="text-background w-full">
                                                ...
                                                <span className="font-uncial-antiqua text-lg pe-[8px]">Description :</span> {item.description ? item.description : '/'} <br />
                                                <span className="font-uncial-antiqua text-lg pe-[8px]">Poids :</span> {item.weight ? item.weight : 0} kg
                                            </span>
                                        </div>
